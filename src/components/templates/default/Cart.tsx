...
const Cart: React.FC<CartProps> = ({ 
  ...
  const { selectedServices, removeService, getTotalDuration, getTotalPrice } = useApp();
    ...
    <div className="w-full bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 h-auto">
      ...
      <div className="border-t border-b border-gray-200 dark:border-gray-700 py-4 mb-4">
        ...
        {selectedServices.length === 0 ? (
          <div className="text-gray-500 dark:text-gray-400 text-center py-6">
            ...
            <p className="text-sm mt-1">Select services from the list to add them to your appointment</p>
          </div>
        ) : (
            ...
            {selectedServices.map(service => (
              <li key={service.id} className="flex justify-between items-center py-2">
                ...
                <div className="flex items-center">
                  <span className="text-gray-900 dark:text-white font-medium mr-3">${service.price}</span>
                  {showButtons && (
                    <button 
      ...
      {showButtons && (
        <Button
          ...
          variant={selectedServices.length === 0 || disabled ? "secondary" : "default"}
        >
          {continueButtonText}
        </Button>
      )}
