export type GeneratePolicyType =
  | "all"
  | "deposit"
  | "cancellation"
  | "rescheduling"
  ...
  if (type === "all" || type === "cancellation") {
    ...
    if (!bs.cancellation_allowed) {
      ...
      policies.push({
        type: "cancellation",
        policy:
          "Cancellations are allowed at any time prior to the appointment without notice.",
      });
    } else {
      policies.push({
        type: "cancellation",
      ...
      policies.push({
        ...
        policy: `A rescheduling attempt must be made at least ${
          ...
          bs.reschedule_penalty_enabled
            ? "to avoid a fee"
            : "else a new booking must be placed."
        }.`,
      });
      if (bs.reschedule_penalty_enabled) {
        policies.push({
          type: "rescheduling",
